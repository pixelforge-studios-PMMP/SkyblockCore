...
class Skeleton extends VanillaEntity
{
    ...
    private function attackPlayer(Player $player): void
    {
        ...
        if ($distSq < self::ATTACK_RANGE_SQ) {
            ...
            if ($currentTime - $this->lastAttackTime >= self::ATTACK_COOLDOWN) {
                $array = array(0.4, 0.7, 0.5, 0.8, 1);
                $value = $array[array_rand($array)];
                $entity = new Arrow(Location::fromObject($this->getEyePos(), $this->getWorld(), ($this->getLocation()->yaw > 180 ? 360 : 0) - $this->getLocation()->yaw, -$this->getLocation()->pitch), $this, $value >= 1);
                $entity->setMotion($this->getDirectionVector());
                $entity->setMotion($entity->getMotion()->multiply($value * 3));
                $entity->spawnToAll();
    ...
    {
        return 0; // Damage handled by arrow
    }

    public function getDrops(): array
    {
        $drops = [
            VanillaItems::BONE()->setCount(random_int(0, 2))
        ];
